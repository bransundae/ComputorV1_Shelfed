...
static int      delim(char *str, char **delim)
{
        int count = 0;
        while(delim[count]){
...
static int      count_words(char *str, char **args)
{
        int     count;
...
static char     *malloc_word(char *str, char **args)
{
        int     i;
        char    *word;
...
char            **signs(char **terms){
}

void            standardize(char *str){
        char *args[] = {"*", "/" "+", "-", "^", "X", NULL};
        char **terms = half_split(str);
        char **left = ft_split(terms[0], args);
        char **right = ft_split(terms[1], args);
